...
public class PlantWateringService extends IntentService {
    ...
    public static final String EXTRA_PLANT_ID = "com.eaxample.android.mygarden.extra.PLANT_ID";
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (ACTION_WATER_PLANT.equals(action)) {
                // 2nd argument is the value to be returned when no value was found
    ...
    private void handleActionWaterPlant(long plantId) {
        // Build the uri for a single plant
    ...
    private void handleActionUpdatePlantWidgets() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            int plantIndex= cursor.getColumnIndex(PlantContract.PlantEntry._ID);
            int createTimeIndex = cursor.getColumnIndex(PlantContract.PlantEntry.COLUMN_CREATION_TIME);
            int waterTimeIndex = cursor.getColumnIndex(PlantContract.PlantEntry.COLUMN_LAST_WATERED_TIME);
            int plantTypeIndex = cursor.getColumnIndex(PlantContract.PlantEntry.COLUMN_PLANT_TYPE);
            long timeNow = System.currentTimeMillis();
            plantId= cursor.getLong(plantIndex);
            long wateredAt = cursor.getLong(waterTimeIndex);
            long createdAt = cursor.getLong(createTimeIndex);
            int plantType = cursor.getInt(plantTypeIndex);
            cursor.close();
            // the canBeWatered boolean is used to determine if we will show or not the drop in the
            // widget
            canBeWatered = (timeNow - wateredAt) > PlantUtils.MIN_AGE_BETWEEN_WATER &&
        ...
        PlantWidgetProvider.updatePlantWidgets(this, appWidgetManager, imgRes, plantId, appWidgetIds,canBeWatered);
    }
}

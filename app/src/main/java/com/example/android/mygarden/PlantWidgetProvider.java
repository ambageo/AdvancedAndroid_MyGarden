...
public class PlantWidgetProvider extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        // we set the action that we want
        wateringIntent.setAction(PlantWateringService.ACTION_WATER_PLANTS);
        // It's a pending Intent to start a service, so here we use getService()
        PendingIntent wateringPendingIntent = PendingIntent.getService(context,
                0,
                wateringIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        // Lastly we handle the click event
        views.setOnClickPendingIntent(R.id.widget_water_plant, wateringPendingIntent);
